    ...
    methods: {
        async getList () {
			const { data: res } = await this.axios({
				method:'get',
				url:'http://127.0.0.1:8000/meeting/getManagerNotApprovedMeetingList',
        ...
        async accept (id) {
			...
			this.accForm.applicationId=id;
			const { data: res } = await this.axios({
				method:'get',
				url:'http://127.0.0.1:8000/meeting/meetingAccept',
				params:this.accForm
			})
        ...
        async rej () {
            ...
            this.$refs.rejFormRef.validate(async valid => {
				...
				const { data: res } = await this.axios({
					method:'get',
					url:'http://127.0.0.1:8000/meeting/meetingReject',
					params:this.rejForm
				})
                // console.log(res)
                if (res.code !== 100) {
