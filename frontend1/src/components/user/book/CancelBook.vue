    ...
    methods: {
        async getList () {
            const { data: res } = await this.axios({
				method:'get',
				url:'http://127.0.0.1:8000/meeting/getMeetingList',
        ...
        async remove (id) {
            ...
            const { data: res } = await this.axios({
				method:'get',
				url:'http://127.0.0.1:8000/meeting/deleteMeeting',
				params: {applicationId: id,username:window.sessionStorage.getItem('username')}
			})
            if (res.code !== 100) {
